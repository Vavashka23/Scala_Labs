...
object Fifth{
  def main(args: Array[String]): Unit = {
    ...
    println(lowNumNotRec(list1, list2))
    println(lowNumTailRec(list1))
  ...
  def lowNumNotRec(oldList: mutable.MutableList[Int], newList: mutable.MutableList[Int]): mutable.MutableList[Int] = {
    for(i <- 2 until oldList.size){
      if(oldList(i) < (oldList(i - 2) - oldList(i - 1))) newList += oldList(i)
    }
    newList
  }

  def lowNumTailRec(oldList: mutable.MutableList[Int]): mutable.MutableList[Int] = {
    @tailrec
